...
// @acess   Public
router.get('/search/users/:username', async (req, res) => {
    try {
        const userRegex = new RegExp(req.params.username, 'i');
...
// @route   PUT /movies/review/likes/:id
// @desc    Add like to review
// @acess   Private
// @todo 	Make private
router.put('/movies/review/likes/:id', async (req, res) => {
    try {
        ...
        if (exist) {
            //if user already liked remove from array
            await review.likes.splice(arrayEl, 1);
        } else {
            // else add like to array
